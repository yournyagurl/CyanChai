...
def initialize_database():
    connection = sqlite3.connect("clover.db")
...
''')
    ...
    cursor.execute('''
            CREATE TABLE IF NOT EXISTS Pets (
                ...
                BreedId INTEGER UNIQUE NOT NULL,
                ...
                FOREIGN KEY (MemberId) REFERENCES Stats(MemberId)
                FOREIGN KEY (BreedId) REFERENCES PetType(BreedId)
            )
        ''')
...
# ADD BREED
def add_breed(breed_name, attack_name, damage_points):
    try:
        connection = sqlite3.connect("clover.db")
        cursor = connection.cursor()

        cursor.execute("INSERT INTO PetBreed (BreedName, Attack, Damage) VALUES (?, ?)", (breed_name, attack_name, damage_points))
        connection.commit()

        cursor.close()
        connection.close()
        print(f"Successfully added {breed_name}!")
...
# ADD PET
def add_pet_to_store(pet_name, pet_type, image_url):
    try:
        connection = sqlite3.connect("clover.db")
        cursor = connection.cursor()

        cursor.execute("INSERT INTO PetStore (PetName, PetType, image_url) VALUES (?, ?)", (pet_name, pet_type, image_url))
        connection.commit()
...
def adopt_pet(member_id, pet_name, pet_type, image_url, adoption_date, level):
    try:
        connection = sqlite3.connect("clover.db")
        cursor = connection.cursor()

        cursor.execute("INSERT INTO Pets (MemberId, PetName, PetType, image_url, AdoptionDate, Level) VALUES (?, ?, ?, ?, ?)",
                       (member_id, pet_name, pet_type, image_url, adoption_date, level))
        connection.commit()
